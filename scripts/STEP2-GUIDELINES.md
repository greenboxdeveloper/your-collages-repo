...
_cairosvg_import_error: str | None = None
try:
    import cairosvg
except (ImportError, OSError) as e:
    cairosvg = None
    _cairosvg_import_error = str(e)
...
def generate_shape_store_manifest(
    ...
    *,
    ...
    if shape_svg_count == 0:
        ...
        detail = f" ({_cairosvg_import_error})" if _cairosvg_import_error else ""
        print(
            "[shape-manifest] WARNING: cairosvg unavailable — skipping WebP files. "
            "Fix: pip install -r scripts/requirements.txt; on Ubuntu CI also "
            "apt-get install -y libcairo2 libcairo2-dev libffi-dev pkg-config."
            f"{detail} "
            "Manifest previewWebpUrl still emitted if --base-url set.",
            file=sys.stderr,
        )
